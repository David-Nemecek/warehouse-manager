...
void MainWindow::addConnection()
{
    ...
    if(!existed){
        ...
        QSqlQuery q("", db);
        q.exec("create table products (id integer primary key, ProductName varchar, StartingInventory integer, InventoryReceived integer, InventoryShipped integer, InventoryOnHand integer, MinimumRequired integer)");
        ...
        q.exec("INSERT INTO `products` (`id`, `ProductName`, `StartingInventory`, `InventoryReceived`, `InventoryShipped`, `InventoryOnHand`, `MinimumRequired`) " \
               "VALUES " \
                   "(1,'Dell Server',100007,35,25,100017,15), " \
                   "(2,'Google books',100,75,654,-479,20), " \
                   "(3,'Cisco Router',45,143,76,86,88), " \
                   "(4,'Keyboard',25,0,0,25,10), " \
                   "(5,'Mouse',1,12,5,8,5), " \
                   "(6,'router',5,23,0,28,1), " \
                   "(7,'Monitors',0,0,0,0,0), " \
                   "(8,'TEST',10,0,0,10,10), " \
                   "(9,'Headphones',500,412,267,6450,400), " \
                   "(10,'Multimeter',3,28,29,2,4), " \
                   "(11,'Motherboard',0,0,300,-300,0), " \
                   "(12,'UniBox',200,0,0,200,300), " \
                   "(13,'Desk',50,1234,0,1284,10), " \
                   "(14,'Toby',567,22,12,577,56467), " \
                   "(15,'Chair',12,0,0,12,12), " \
                   "(16,'test',500,12,0,512,25), " \
                   "(17,'Firewalls',5,33,900,-862,10), " \
                   "(18,'Cables',9,475,9000,16,100), " \
                   "(19,'Test',25,0,0,25,222);");
...
void MainWindow::on_pushButton_filterProducts_clicked()
{
    ...
    model->setQuery(QSqlQuery("SELECT products.productName 'Product Name', startingInventory 'Starting Inventory', "
                              ...
                              "ioh 'Inventory On Hand', minimumRequired 'Minimum Required' FROM products "
                              "LEFT JOIN (SELECT productName, IFNULL( StartingInventory + "
                              "IFNULL((SELECT SUM(numberReceived) FROM purchases WHERE productId = products.id AND (purchaseDate BETWEEN '" + dateFrom + "' AND '" + dateTo + "')),0) - "
                              "IFNULL((SELECT SUM(numberShipped) FROM orders WHERE productId = products.id AND (orderDate BETWEEN '" + dateFrom + "' AND '" + dateTo + "')), 0), 0) AS 'IOH' "
                              "FROM products "
                              "LEFT JOIN purchases ON products.id = purchases.productId "
                              "LEFT JOIN orders ON products.id = orders.productId "
                              "GROUP BY productName) t ON products.productName = t.productName", db));
